...
const FutureSkillReportPage = () => {
  ...
  const trends = [
    {
      ...
      points: ["Legal AI Market: 13.1% CAGR to 2035 (USD 2.1B → 7.4B)", "53% of Am Law 200 Firms already use Legal AI", "Contract Review: 85% time savings possible (92 min → 26 sec)"],
      impact: "Prompt Engineering & Validation become baseline skills",
    },
    {
      title: "Regulatory Intensification",
      points: ["EU AI Act Enforcement 2026 (Penalties: EUR 35M or 7% revenue)", "ESG Regulation (CSDRD) mandatory since 2024", "GDPR 2025 with AI-specific rules"],
      impact: "Compliance skills become deal standard",
    },
    {
      title: "Labor Market Shift",
      points: ["Top 3 Skills 2025: Commercial Awareness, Tech Proficiency, Regulatory Expertise", "AI Proficiency: +20-30% Salary Premium", "79% of Law Firms expect transformational AI impact"],
      impact: "Tech literacy becomes career accelerator",
    },
    {
      title: "M&A Market Expansion",
      points: ["2024: +12% Deal Value, +20% in Large Deals", "Tech M&A: +16% ($640B) - AI/Cloud/Cyber Acquisitions", "Deal Complexity: ESG, AI, Multi-Regulatory, Remote Teams"],
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Critical Insights */}
      <Card className="bg-card/80 border-border/50 border-l-4 border-l-primary">
        <CardHeader>
          ...
          <div className="grid md:grid-cols-2 gap-6">
            {trends.map((trend, i) => (
              <div key={i} className="p-4 rounded-lg bg-secondary/30 border border-border/50">
                ...
                <p className="text-sm font-medium text-primary">Impact: {trend.impact}</p>
              </div>
            ))}
          </div>
        </CardContent>
      ...
      {/* Risk Warning */}
      <Card className="bg-card/80 border-border/50 border-l-4 border-l-[hsl(var(--skill-weak))]">
        <CardHeader>
          <CardTitle className="text-foreground flex items-center gap-2">
          ...
          <div className="grid md:grid-cols-2 gap-6">
            <div>
              ...
              <ul className="space-y-2">
                {["-40-50% productivity vs. AI-competent peers", "Partner transition problematic", "Compliance gaps in regulated deals", "Clients switch to modern lawyers", "Salary stagnation (vs. +20-30% AI premium for others)"].map((item, i) => (
            ...
            <div>
              ...
              <ul className="space-y-2">
                {["+40-50% deal efficiency", "+10-15% accuracy (AI-human collaboration)", "100% AI Act/ESG/GDPR compliance", "Multi-jurisdictional deal leadership", "Partner track accelerated"].map((item, i) => (
                  <li key={i} className="text-sm text-muted-foreground flex items-center gap-2">
